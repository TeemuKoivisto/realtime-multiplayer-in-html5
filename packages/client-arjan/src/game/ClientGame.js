...
import { AbstractGame } from '@example/game-arjan'
import { ClientPlayer as Ghost } from './ClientPlayer'
...
export function ClientGame({ options }) {
  ...
  function addPlayer(player) {
    ...
    const serverGhost = Ghost({ x, y, width: player.getWidth(), height: player.getHeight() })
    const localGhost = Ghost({ x, y, width: player.getWidth(), height: player.getHeight() })
  ...
  function processNetworkUpdates(interpolation) {
    ...
    if (!options.clientPrediction && !options.naiveApproach) {
      ...
      ghosts.server.setPosition(
        latestServerUpdate.ownPlayer.position.x,
        latestServerUpdate.ownPlayer.position.y
      )
