...
export class GameClientV2 extends Game {
  ...
  constructor(playerId: string, viewport: HTMLCanvasElement) {
    super()
  ...
  client_process_net_prediction_correction() {
    ...
    const my_server_pos = latest_server_data.players.find(p => p.playerId === this.playerId)
  ...
  client_update_local_position(p: PlayerV2) {
    if (this.client_predict) {
      ...
      //p.pos = this.v_add( old_state, this.v_mul_scalar( this.v_sub(current_state,old_state), t )  );
      p.pos = current_state
  ...
  client_create_ping_timer() {
    ...
    setInterval(() => {
      ...
      this.emit(ClientMessageType.ping, { ping: this.last_ping_time })
    }, 1000)
  }
  ...
  on_client_ready(data: ServerMessage[ServerMessageType.client_ready]) {
  ...
  on_client_join_game(data: ServerMessage[ServerMessageType.client_join]) {
    ...
    this.players = this.players.map(p => {
      if (p.playerId === data.playerId) {
        p.info_color = data.color
        p.state = 'connected.joined.waiting'
        p.online = true
      }
      return p
    })
  ...
  on_client_ping(data: ServerMessage[ServerMessageType.client_ping]) {
    this.net_ping = new Date().getTime() - parseFloat(data.ping)
